...
{
	u_long value;
	value = le32_to_cpu(*(volatile u_long *)RALINK_REG_PIODIR);
	value &= ~(0xffffffff);
	*(volatile u_long *)(RALINK_REG_PIODIR) = cpu_to_le32(value);
	return le32_to_cpu(*(volatile u_long *)RALINK_REG_PIODATA);
}
...
{ \
	...
	val |= (1 << 4 * (last_port_id - 1)); \
	mii_mgr_write(0x1f, reg, val); \
}

#define last_led_reg_bit_off(reg) \
{ \
	u32 val; \
	mii_mgr_read(0x1f, reg, &val); \
	val &= ~(1 << 4 * (last_port_id - 1)); \
...
*/
	...
	if (ubnt_bd_valid) {
		for (rcount = 0; rcount <= CONFIG_FACTORY_RESET_TIME; rcount++) {
			...
			printf("%s...%d", (rcount == 0) ? "Reset pressed " : "", rcount);
